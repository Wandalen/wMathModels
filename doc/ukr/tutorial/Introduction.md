...
Рутина `make` дозволяє створити екземпляр моделі. Новий екземпляр можливо створити на основі заданої розмірності або іншого екземпляра. Якщо в якості аргумента використано екземпляр існуючої моделі, то новий екземпляр буде копією оригінального екземпляра.
...
Якщо ж у виклик `from` передати `null`, то отримаємо таку ж семантику як в випадку із рутиною `make`.
...
Відмінність між рутинами `make()` та `from()` в тому, як вони реагують на готові екземпляри в якості аргумента.
...
Дані екземплярів всіх математичних моделей цього модуля зберігаються в **векторі** ( будь-якої форми ) або **матриці**. Контейнером для моделі бокс, кватерніон, пряма та багато інших є **вектор**.
...
<!-- Dmytro : може дати посилання на модуль wMathVector -->
...
Всі математичні моделі в цьому модулі ( як векторні, так і матричні ) складаються із атомарних частин, на найнижчому рівні це скаляри, які в свою чергу часто групуються в більші групи.
...
Наприклад, алгоритм перевірки того, що точка знаходиться на поверхні або всередині реалізований рутиною `pointContains`. Всі моделі для яких можливо реалізувати такий алгоритм мають таку рутину і вона називаєтсья саме так.
...
Створюється опуклий полігон `polygon` в 2D на основі координат вершин із вектора `vertices`. В змінну `contains` повертається `true` так, як точка `point` та третя вершина полігону `polygon` мають однакові координати.

### Intuitive

Рутини маю інтуїтивні назви. Знання про одну рутину допомагає здогадатися про інші рутини. Пошук та навчання <!-- Dmytro : навчання чого? --> можливо здійснювати комбінуванням префісів/суфіксів.
...
В 2D випадку сфера це коло. Так само як в 3D випадку описується центр та радіус, на що потрібно 3 скаляри.
...
В 3D випадку сфера описується 4-ма скалярами.
