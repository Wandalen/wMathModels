...
Рутина `make` дозволяє створити екземпляр моделі. Новий екземпляр можливо створити на основі заданої розмірності або іншого екземпляра. Якщо в якості аргумента використано екземпляр існуюючої моделі, то новий екземпляр буде копією оригінального екземпляра.

Cтворення екземпляра моделі `box` із розмірністю за замовчуванням:
...
Бокс `box` знаходиться в `3D` просторі. Три перших скаляри вектора `box` описують одну крайню точку тоді як три останніх скаляри описують іншу крайню точку. Це є параметрами моделі бокс за замовучанням.
...
Виклик рутини `make` із скаляром створить бокс заданої розмірність.
...
Виклик `_.box.make( srcBox )` створює і повертає вектор, котрий маістить 4 скаляри, по 2-ва на точку. Дві точки описують бокс.
...
Альтернативним способом створити екземпляр метематичної моделі є використання рутини `from`. Рутина `from` так само, як і `make` конструює новий екземпляр, але на відміну від рутини `make` ще робить додаткову перевірку. Рутина `from` створює новий екземпляр лише якщо аргументом виклику не є екземпляром даної моделі. Якщо ж на вхід рутини `from` передається екземпляр даної моделі тоді він же повертається без жодних змін.
...
Рутина `_.box.from()` в першу чергу перевіряє чи `srcBox` не є екземпляром моделі `box`. І `srcBox` є екземпляром моделі `box` тому не вносячи жодних змін він же і повертається із рутини `_.box.from()`.

Альтернативно рутина `from()` прийме і вектор адаптер.
...
Так само як в попередньому прикладі рутина `_.box.from()` повертається `srcBox` без жодних змін.
...
Два альтернативних способи `make()` та `from()` для стоврення екземплярів математичних моделнй мають схожості та відмінності.
...
Відмінніст між рутинами `make()` та `from()` в тому, як вони реагують на готові екземпляри в якості аргумента.
...
Дані екземплярів всіх математичних моделей цього модуля зберігаються в **векторі** ( будь-якої форми ) або **матриці**. Контейнером для моделі бокс, кватерніон, пряма та багато інших є **вектором**.
...
Контейнером для моделі полігон та frustum є **матриця**. Матриця здатна переносити багатовимірну інформацію, що є необхідністю для таких моделей, як полігон.
...
Розділення даних та функціоналу:
 ...
 - що в свою чергу робить систему більш розширюваною.
...
Всі алгоритми моделей викликаються через немйспейс моделі, напрмклад для бокса це є `_.box.*`, для кватерніона `_.quat.*` тоді, як екземпляр моделі є **вектором** або **матрицею**, а не об'єктом якогось додаткового класа. Всі алгоритми математичної моделі викликаються функціонально, наприклад:
...
Всі математичні моделі в цьому модулі ( як векторні так і в матричні ) складаються із атомарних частин, на найнищому рівні це скаляри, які в свою чергу часто групуються в більші групи.
...
Вручну створюється екземпляр моделі бокс `box`. Рутиною `_.box.cornerLeftGet( box )` створюється вектор адаптер на ліву нижню точку бокса і далі вивід її в лог.
...
Вектор адаптер `cornerLeft` використано для того щоб змінити значення першої вершини бокса.
...
Наприклад, алгоритм перевірки точо чи точка знаходиться всередині реалізований рутино `pointContains`. Всі моделі для яких можливо реалізувати такий алгоритм мають таку рутину і вона називаєтсья саме так.
...
Створюється опуклий полігон `polygon` в 2D на основі координат вершин із вектора `vertices`. В змінну `contains` повертається `true` так, як точка `point` та третя вершина полігону `polygon1` мають однакові координати.

### Intuitive

Рутини маю інтуїтивні назви. Знання про одну рутину допомагає здогадатися про інші рутини. Пошук та навчання можливо здійснювати комбінуванням префісів/суфіксів.

Приклад використання групи рутин `*Intersects` для перевірки перетину екземпляра моделі `plane` із екземплярами інших моделей.
...
В змінну `intersected` повертається `true` так, як площина `plane` перетинає зрізану піраміду `frustum`.
...
Рутини, які очікують `dst` контейнер першим аргументом, можуть створювати новий інстанс моделі замість того щоб переписувати існуючий. `dst` це аргумент в який виконується запис, якщо здійснюється якийсь запис. `dst` йде першим аргументом. `null` в якості першого аргумента дає інструкцію створити новий контейнер для запису результату.
...
Екземпляр `box` моделі `box` створюється із точок `point1` та `point2`. З виводу видно, що `box` містить точки
`point1` та `point2`.

Альтернативно, можливо створити контейнер для запису резултату вручну і передати його першим аргументом:
...
Створюється вектор `dstBox`, який стане контейнером для даних екземпляра моделі `box`. `_.box.makeSingular` заповнює `dstBox` безкінечностями. На основі точок `point1` та `point2` розраховуються крайні точки бокса. В результаті ліва-нижня точка бокса має знчення `( 0, 1 )`, а права-верхня має значення `( 3, 8 )`.

### Naming pattern

Зверніть увагу на патерн за яким рутини отимують свої назви:
...
Один і той же інтерфейс приховує реалізації алгоритмів для різних ромірностей: 2D, 3D, 4D...
...
В 2D випадку сфера це коло. Так само як в 3D випадком описується центром та радіс, на що потрібно 3 скаляри.
...
В 3D випадку сфера описується 4-ма скаляри.
...
В змінну `distance` повертається відстань від точки `point` до площини `plane`, яка ' `-0.27`. Простір 3D.

### Alternative models

Екземпляр моделі може бути перетворений із однієї концептуальної форми в іншу, альтернативну.

Повороти можуть бути задані кватерніоном, кутами еулера, віссю обертання та поворотом чи матрицею. Пряма може бути задана неявним рівнянням, двома точками, точкою та відносним напрямком і тд. Для кожної концепції можливо відшукати декілька альтернетивних математичних моделей. Модуль реалізує деякі із таких альтернатив та дає засоби конвертування екземпляр одної моделі в екземпляр іншої.
...
Створюються дві лінії за двома точками `linePoints1` та `linePoints2` на основі координат переданих як вектори. Рутина `_.linePoints.pairIntersectionPoint` вираховує координати точки перетину і записує їх в змінну `point1`. Далі відбувається конвертування в модель `linePointDir`. `linePointsDir1` та `linePointsDir1` є екземплярами моделі `linePointDir`. `linePointsDir1` та `linePointsDir1` створені на основі `linePoints1` та `linePoints2`. Аналогічно для моделі за точкою та відносним напрямком рутина `_.linePointDir.pairIntersectionPoint` вираховує координати точки перетину і записує їх в змінну `point2`. Точки `point1` та `point2` мають одне й теж значення `( 2,2 )` попри те що були вирахувані різними математичними моделями.

Більше про модель `linePoints` можливо почитати [тут.](../concept/Overview.md#LinePoints).
Більше про модель `linePointDir` можливо почитати [тут.](../concept/Overview.md#LinePointDir).

Приклад перетворення кутів уелера в кватерніони та назад:
...
Створюється вектор `euler1` який є контейнер екземпляра математичної моделі кутів еулера. В змінну `quat` записується кватерніон, який є результатом перетворення із кутів еулера `euler1`. В змінну `euler2` записується значення кутів еулера, які є результатом перетворення кватерніону `quat`. З виводу видно що трансформація, яка описується кутами еулера на початку прикладу збереглася.
