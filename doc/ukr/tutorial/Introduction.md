...
Як через матрицю так і через вектор адаптер можливо реалізувати zero-copy принцип. Розділення даних так функціоналу дає можливість уникати марного копіювання даних, спрощує використання модуля, стандартизує інтерфейс, що в свою чергу робить систему більш розширюваною.

Алгоритми всіх моделей не привязані до форми та формату даних. Всі рутини моделей викликаються через немйспейс моделі, напрмклад для бокса це є `_.box.*`, для кватерніона `_.quat.*` тоді, як екземпляри моделі є векторами або матирцями, а не обєктами якогось додаткового об'єкта.
