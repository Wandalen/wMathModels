...
Задається двома точками: початком `( 2, 1 )` та кінцем відрізка `( 9, 4 )`. Для опису відрізка у 2D достатньо 4 скалярів.
...
Капсула в прикладі починається в точці `( 2, 1 )`, закінчується в точці `( 9, 4 )` і має радіус `0.5`. Для опису капсули у 2D достатньо 5 скалярів.
...
Модель обрізаної піраміди часто застосовується в 3D графіці для видимої частини простору видимої перспективною камерою. Модель задається 6-ма гранями. Кожна грань, задана неявним рівнянням площини. Екземпляр моделі зберігає свої параметри в матриці розмірності 4x6. Рівняння площини зберігаються в колонках такої матриці.
...
Один із можливих способів задати обертання. Задається трьома кутами повороту навколо 3-х осей та 3-ма осями. Перших 3-ри скаляри вектора, для зберігання даних моделі, є кутами повороту, а останні 3-ри скаляра несуть інформацію про те, навколо яких осей потрібно здійснювати поворот. Всього в векторі 6-ть скалярів. В 3D існує 12 можливих комбінацій послідовності осей:
