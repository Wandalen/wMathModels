...
function toMatrix2( test )
{
  ...
  test.description = 'Euler XYZ - Matrix -> Euler -> Matrix '; //
  ...
  var matrix = _.euler.fromMatrix2( matrix, dst );
  var result = _.euler.toMatrix2( euler );
  test.equivalent( result, expected );

  test.description = 'Euler YXZ - Matrix -> Euler -> Matrix'; //

  var euler = _.Space.make( [ 3, 3 ] ).copy(
       [ 0.7701, -0.4794, 0.4207,
         0.6307, 0.4741, - 0.6142,
         - 0.0950, 0.7384, 0.6675 ] );
  var dst = [ 0, 0, 0, 1, 0, 2 ];
